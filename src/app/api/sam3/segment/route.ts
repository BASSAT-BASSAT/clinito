...
const SAM3_MODE = process.env.NEXT_PUBLIC_SAM3_MODE || 'cloud';

export async function POST(request: NextRequest) {
  // Use cloud mode by default (works on Vercel)
  // Set NEXT_PUBLIC_SAM3_MODE=local to use localhost:8000
  if (SAM3_MODE === 'local') {
    return handleLocalSAM3(request);
  } else {
    return handleCloudSAM3(request);
  }
}

// ============================================
// LOCAL MODE: Forward to localhost:8000
// ============================================
async function handleLocalSAM3(request: NextRequest) {
  try {
...
// ============================================
// CLOUD MODE: Use HuggingFace SAM3 API
// ============================================
async function handleCloudSAM3(request: NextRequest) {
  try {
    const formData = await request.formData();
